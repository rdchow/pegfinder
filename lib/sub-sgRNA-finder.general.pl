...
sub process_chosen_sgRNA_general {
    my ($c_sg,$seq1,$minEditPos,$maxEditPos,$maxEditDistance,$wtdelcounter,$seq2,$rgn) = @_;
    my $chosenOrientation;
    my $chosenSG = $c_sg;
    my $chosenCutPos;
    my $chosenDistance;
    ...
    $counter += () = $seq1 =~ /$sgPAM/;
    $counter += () = (reverse_complement($seq1) =~ /$sgPAM/);
    #print $counter,"\n";
    if ($counter > 1){
        ...
        elsif (reverse_complement($seq1) =~ /$sgPAM/){
        ...
        my $chosenDisrupt;
        if ($chosenOrientation eq "sense"){
            if ($seq2 !~ /$sgPAM/){ #if the sgRNA is disrupted in the edited sequence:
                $chosenDisrupt = 1;
            }
            else {
                $chosenDisrupt = 0;
            ...
            if ($seq2 !~ /$rc_sgPAM/){ #if the sgRNA is disrupted in the edited sequence:
                $chosenDisrupt = 1;
            }
            else {
                $chosenDisrupt = 0;
...
sub find_choose_sgRNA_general {
    my ($seq1,$minEditPos,$maxEditPos,$maxEditDistance, $wtdelcounter,$seq2,$rgn) = @_;
    my $chosenOrientation;
    my $chosenSG;
    my $chosenCutPos;
    my $chosenDistance;
...
sub find_choose_nick_sgRNA_general{
    my ($seq1,$minEditPos,$maxEditPos,$maxEditDistance,$chosenCutPos,$chosenOrientation,$minNickDist,$maxNickDist,$rgn) = @_;
...
sub find_pe3b_sgRNA_general {
    my ($seq1,$minEditPos,$maxEditPos,$maxEditDistance,$chosenCutPos,$chosenOrientation,$seq2,$wtdelcounter,$mutdelcounter,$rgn) = @_;
    my $chosenNickOrientation;
    my $chosenNickSG;
    my $chosenNickSGPos;
    my $chosenNickSGDist;
        ...
        for (my $i = 0; $i < scalar @chars-22; $i++){
            ...
            if (defined $tempsg){
                if (length($tempsg) == 20){
                    ...
                    $tempsg = reverse_complement($tempsg); #sense oreintation of sgRNA
                    my $rc_tempsg = reverse_complement($tempsg); # reverse comp, for string matching purposes 
